...
export async function getDashboard(request: FastifyRequest, reply: FastifyReply) {
  try {
    ...
    const [stocks, mutualFunds, bankAccounts, fixedDeposits, recurringDeposits, bonds, ppfAccounts, npsAccounts, gold, realEstate, epfAccounts, crypto, liabilities] = await Promise.all([
    ...
    // Calculate totals
    const stocksTotal = stocks.reduce((sum: number, s: any) => sum + (s.currentValue || s.investedAmount), 0);
    const mfTotal = mutualFunds.reduce((sum: number, m: any) => sum + (m.currentValue || m.investedAmount), 0);
    ...
    const bondsTotal = bonds.reduce((sum: number, b: any) => sum + (b.currentValue || b.faceValue * b.units), 0);
    ...
    const goldTotal = gold.reduce((sum: number, g: any) => sum + (g.currentValue || ((g.quantityGrams || 0) * (g.averagePricePerGram || 0))), 0);
    const realEstateTotal = realEstate.reduce((sum: number, r: any) => sum + (r.currentValue || r.purchasePrice), 0);
    const epfTotal = epfAccounts.reduce((sum: number, e: any) => sum + e.balance, 0);
    const cryptoTotal = crypto.reduce((sum: number, c: any) => sum + (c.currentValue || c.quantity * c.averagePrice), 0);
    
    // Calculate liabilities
    const totalLiabilities = liabilities.reduce((sum: number, l: any) => sum + l.currentBalance, 0);
    
    const totalAssets = stocksTotal + mfTotal + bankTotal + fdTotal + rdTotal + bondsTotal + ppfTotal + npsTotal + goldTotal + realEstateTotal + epfTotal + cryptoTotal;
...
export async function getAssetAllocation(request: FastifyRequest, reply: FastifyReply) {
  try {
    ...
    const [stocks, mutualFunds, bankAccounts, fixedDeposits, recurringDeposits, bonds, ppfAccounts, npsAccounts, gold, realEstate, epfAccounts, crypto] = await Promise.all([
      prisma.stock.findMany({ where: { userId } }),
      prisma.mutualFund.findMany({ where: { userId } }),
      prisma.bankAccount.findMany({ where: { userId } }),
      prisma.fixedDeposit.findMany({ where: { userId } }),
    ...
    const allocation = [
      { name: 'Stocks', value: stocks.reduce((sum: number, s: any) => sum + (s.currentValue || s.investedAmount), 0) },
      { name: 'Mutual Funds', value: mutualFunds.reduce((sum: number, m: any) => sum + (m.currentValue || m.investedAmount), 0) },
      { name: 'Bank Accounts', value: bankAccounts.reduce((sum: number, b: any) => sum + b.balance, 0) },
      { name: 'Fixed Deposits', value: fixedDeposits.reduce((sum: number, f: any) => sum + f.amount, 0) },
      { name: 'Recurring Deposits', value: recurringDeposits.reduce((sum: number, r: any) => sum + r.maturityAmount, 0) },
      { name: 'Bonds', value: bonds.reduce((sum: number, b: any) => sum + (b.currentValue || b.faceValue * b.units), 0) },
      ...
      { name: 'Gold', value: gold.reduce((sum: number, g: any) => sum + (g.currentValue || ((g.quantityGrams || 0) * (g.averagePricePerGram || 0))), 0) },
      { name: 'Real Estate', value: realEstate.reduce((sum: number, r: any) => sum + (r.currentValue || r.purchasePrice), 0) },
      { name: 'EPF', value: epfAccounts.reduce((sum: number, e: any) => sum + e.balance, 0) },
      { name: 'Crypto', value: crypto.reduce((sum: number, c: any) => sum + (c.currentValue || c.quantity * c.averagePrice), 0) },
...
export async function getPerformance(request: FastifyRequest, reply: FastifyReply) {
  try {
    ...
    const [stocks, mutualFunds, fixedDeposits, epfAccounts, crypto, realEstate, gold, ppfAccounts, npsAccounts] = await Promise.all([
      prisma.stock.findMany({ where: { userId } }),
      prisma.mutualFund.findMany({ where: { userId } }),
      prisma.fixedDeposit.findMany({ where: { userId } }),
      prisma.ePF.findMany({ where: { userId } }),
    ...
    const performance = [
      {
        ...
        current: stocks.reduce((sum: number, s: any) => sum + (s.currentValue || s.investedAmount), 0),
        returns: stocks.reduce((sum: number, s: any) => sum + (s.returns || 0), 0),
        returnsPercentage: stocks.length > 0 
          ? stocks.reduce((sum: number, s: any) => sum + (s.returnsPercentage || 0), 0) / stocks.length 
          : 0,
      },
      {
        name: 'Mutual Funds',
        invested: mutualFunds.reduce((sum: number, m: any) => sum + m.investedAmount, 0),
        current: mutualFunds.reduce((sum: number, m: any) => sum + (m.currentValue || m.investedAmount), 0),
        returns: mutualFunds.reduce((sum: number, m: any) => sum + (m.returns || 0), 0),
        returnsPercentage: mutualFunds.length > 0 
          ? mutualFunds.reduce((sum: number, m: any) => sum + (m.returnsPercentage || 0), 0) / mutualFunds.length 
          : 0,
      },
      {
        name: 'Crypto',
        invested: crypto.reduce((sum: number, c: any) => sum + (c.quantity * c.averagePrice), 0),
        current: crypto.reduce((sum: number, c: any) => sum + (c.currentValue || c.quantity * c.averagePrice), 0),
        ...
        returnsPercentage: crypto.length > 0 
      ...
      },
      {
        ...
        current: realEstate.reduce((sum: number, r: any) => sum + (r.currentValue || r.purchasePrice), 0),
        returns: realEstate.reduce((sum: number, r: any) => {
          const current = r.currentValue || r.purchasePrice;
          return sum + (current - r.purchasePrice);
        }, 0),
        returnsPercentage: realEstate.length > 0 
          ? realEstate.reduce((sum: number, r: any) => {
              const current = r.currentValue || r.purchasePrice;
              return sum + (r.purchasePrice > 0 ? ((current - r.purchasePrice) / r.purchasePrice) * 100 : 0);
            }, 0) / realEstate.length 
          : 0,
      },
      {
        name: 'Gold',
        invested: gold.reduce((sum: number, g: any) => sum + ((g.quantityGrams || 0) * (g.averagePricePerGram || 0)), 0),
        current: gold.reduce((sum: number, g: any) => sum + (g.currentValue || ((g.quantityGrams || 0) * (g.averagePricePerGram || 0))), 0),
        ...
        returnsPercentage: gold.length > 0 
