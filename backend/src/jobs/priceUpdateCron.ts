  ...
  try {
    ...
    const updatePromises = stocks.map(async (stock: Stock) => {
      ...
      if (currentPrice) {
        ...
        const { returns, returnsPercentage } = calculateSimpleReturns(stock.investedAmount, currentValue);
  ...
  try {
    ...
    const updatePromises = mutualFunds.map(async (mf: MutualFund) => {
      ...
      if (navData) {
        ...
        const { returns, returnsPercentage } = calculateSimpleReturns(mf.investedAmount, currentValue);
...
async function updateGoldPrices() {
  try {
    ...
    const physicalDigitalGold = goldAssets.filter((g: Gold) => g.type === 'physical' || g.type === 'digital');
    ...
    console.log(`Updated ${goldAssets.length} gold prices (${physicalDigitalGold.length} physical/digital, ${goldETFs.length} ETFs)`);
  } catch (error) {
